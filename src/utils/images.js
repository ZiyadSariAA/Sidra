...
// دالة للحصول على صورة احتياطية
export const getFallbackImage = (type = 'article') => {
  const fallbackImages = {
    article: '/assets/placeholder-article.jpg',
    host: '/assets/placeholder-host.jpg',
    episode: '/assets/placeholder-episode.jpg',
    series: '/assets/placeholder-series.jpg'
  };
  
  return fallbackImages[type] || fallbackImages.article;
};

// دالة لتحميل الصورة مع معالجة الأخطاء
export const loadImage = (src, fallbackSrc = null) => {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      if (fallbackSrc) {
        ...
        reject(new Error(`فشل في تحميل الصورة: ${src}`));
      }
    };
